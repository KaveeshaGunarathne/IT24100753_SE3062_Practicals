...
class VisualGridHuntGame:
    ...
    def get_percept(self) -> dict:
        hx, hy = self.agent_pos[0], self.agent_pos[1] + 1
        wall_ahead = (hx < 0 or hx >= self.width or hy < 0 or hy >= self.height or (hx, hy) in self.walls)
        return {
    ...
    def draw_grid(self):
        ...
        for tx, ty in self.env.toxic_traps:
            ...
            self.canvas.create_polygon(
                x1 + self.cell_size / 2, y1 + offset,
                x1 + offset, y1 + self.cell_size - offset,
                x1 + self.cell_size - offset, y1 + self.cell_size - offset,
                fill="purple", outline="indigo"
            )
